...
function ContactLensContent() {
  ...
  const [selectedLensColor, setSelectedLensColor] = useState("black");
  const [prescription, setPrescription] = useState("");
  const [water, setWater] = useState("");
  const [baseCurve, setBaseCurve] = useState("");
  const [siteContent, setSiteContent] = useState<any>(null);

  const lensColors = ["Black", "Brown", "Blue", "Green", "Hazel", "Gray"];
      ...
      <main className="min-h-screen bg-white">
        ...
        <div className="border-b border-gray-200">
          <div className="mx-auto max-w-[1440px] px-5 py-6 sm:px-8 lg:px-12">
            ...
            <h1 className="text-3xl font-bold text-gray-950">Contact Lens</h1>
            <p className="mt-2 text-gray-600">
              Browse our premium contact lens collection with customizable colors and prescriptions
            </p>
          </div>
        </div>
          ...
          <div className="mx-auto max-w-[1440px] px-5 py-6 sm:px-8 lg:px-12">
            ...
            >
              <div className="text-left">
                <h3 className="font-semibold text-gray-950">Customize Your Contact Lens</h3>
                <p className="text-sm text-gray-600">Select color, prescription, and specifications</p>
              </div>
              <ChevronDown
                size={20}
            ...
            {showLensOptions && (
              <div className="mt-4 rounded-lg border border-gray-200 bg-white p-6 animate-in fade-in">
                ...
                <div className="mt-6 rounded-lg bg-gray-50 p-4">
                  <p className="text-sm font-medium text-gray-950 mb-2">Selected Options:</p>
                  <div className="flex flex-wrap gap-2">
                    <span className="inline-block rounded-full bg-white px-3 py-1 text-xs font-medium text-gray-950 border border-gray-300">
                      Color: {selectedLensColor.charAt(0).toUpperCase() + selectedLensColor.slice(1)}
                    </span>
                    {prescription && (
                      <span className="inline-block rounded-full bg-white px-3 py-1 text-xs font-medium text-gray-950 border border-gray-300">
                        Rx: {prescription}
